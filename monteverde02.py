...
Original file is located at
    https://colab.research.google.com/drive/1nyJA9WSNNtB4qTdvHzCM7Ip2llAshrQM
...
nlp = stanza.Pipeline('en', processors='tokenize,pos,lemma,depparse')
cleaned_text = download_and_clean_gutenberg_book(book_id='Moby_Dick')  # Replace book_id with desired value
...
# Example usage:
process_and_save_to_json(book_id='Moby_Dick', output_filename="nlp_output.json")  # Replace book_id and filename
...
def descargar_moby_dick():
    """Descarga el texto de Moby Dick desde Project Gutenberg."""
    url = "https://www.gutenberg.org/files/2701/2701-0.txt"
    response = requests.get(url)
    return response.text

def extraer_entidades(texto):
    """Extrae entidades nombradas del texto utilizando Stanza."""
    ...
    for sent in doc.sentences:
        for ent in sent.ents:
            entidades.append((ent.text, ent.type))
    return entidades

# Descargar el texto de Moby Dick
texto_moby_dick = descargar_moby_dick()

# Extraer las entidades
entidades = extraer_entidades(texto_moby_dick)
...
def extraer_entidades(texto):
    """Extrae entidades nombradas del texto utilizando Stanza y las devuelve como una lista de diccionarios."""
    nlp = stanza.Pipeline('en', processors='tokenize,ner')
    doc = nlp(texto)
    ...
    for sent in doc.sentences:
        for ent in sent.ents:
            entidades.append({"text": ent.text, "type": ent.type})
    return entidades
...
# Guardar las entidades en un archivo JSON
with open('entidades_moby_dick.json', 'w') as f:
    json.dump(entidades, f, indent=4)

print("Las entidades se han guardado en el archivo entidades_moby_dick.json")
...
def process_with_stanza(text, book_title="Moby Dick"):
    ...
    # Guardar los resultados en un archivo JSON
    with open(f"{book_title}_analysis.json", 'w') as f:
        json.dump(results, f, indent=4)
...
# Ejemplo de uso:
book_id = 1342
text = download_and_clean_gutenberg_book(book_id)  # Make sure download_and_clean_gutenberg_book is defined
analysis = process_with_stanza(text)
print("Análisis guardado en Moby Dick_analysis.json")
...
def align_tags(stanza_tags, reference_tags):
    """
    Aligns Stanza tags with reference tags for evaluation.

    This is a placeholder. Implement your actual alignment logic here.

    Args:
        stanza_tags: List of tags from Stanza.
        reference_tags: List of reference tags.

    Returns:
        Tuple of aligned Stanza tags and aligned reference tags.
    """
    # Basic alignment (assumes same length for simplicity)
    # Replace with more robust alignment if necessary
    aligned_stanza_tags = stanza_tags[:min(len(stanza_tags), len(reference_tags))]
    aligned_reference_tags = reference_tags[:min(len(stanza_tags), len(reference_tags))]
    return aligned_stanza_tags, aligned_reference_tags


# Get reference tags from Treebank
reference_sentences = treebank.tagged_sents()
reference_tags = [tag for sentence in reference_sentences for _, tag in sentence]

# Get Stanza tags
text = download_gutenberg_book(1342)  # Get text from Gutenberg
nlp = stanza.Pipeline('en', processors='tokenize,pos')
doc = nlp(text)
stanza_tags = [word.upos for sentence in doc.sentences for word in sentence.words]

# Align tags
aligned_stanza_tags, aligned_reference_tags = align_tags(stanza_tags, reference_tags)

# Calculate and print the classification report
print(classification_report(aligned_reference_tags, aligned_stanza_tags))  # Corrected argument order
...
# Descarga de Moby Dick
def download_gutenberg_book(book_id):
    """
    Descarga un libro de Proyecto Gutenberg y lo limpia.

    Args:
        book_id: El ID del libro en Proyecto Gutenberg (900 para Moby Dick).
...
# Texto de Moby Dick
text = download_gutenberg_book(900)  # Moby Dick ID
