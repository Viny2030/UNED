...
# Cargar el modelo de Transformers (ejemplo: BERT para análisis de sentimiento)
tokenizer = AutoTokenizer.from_pretrained('bert-base-uncased')
model = AutoModelForSequenceClassification.from_pretrained('bert-base-uncased')

def extraer_caracteristicas(texto):
    """
    Extrae características del texto utilizando Stanza y Transformers.

    Args:
        texto: El texto a analizar.

    Returns:
        Un diccionario con las características extraídas.
    """
    ...
    caracteristicas["dependencias"] = []
    for oracion in doc.sentences:
        for palabra in oracion.words:
            for dependencia in oracion.dependencies:
                if dependencia[1] == palabra.id:
                    caracteristicas["dependencias"].append(dependencia)
    ...
    caracteristicas["sentimiento"] = etiqueta_predicha
...
with open("caracteristicas_texto.json", "w") as archivo:
    json.dump(caracteristicas, archivo, indent=4)
...
Importar bibliotecas: Se importan las bibliotecas necesarias: stanza para el procesamiento del lenguaje natural, torch para trabajar con tensores, y AutoTokenizer y AutoModelForSequenceClassification de la biblioteca transformers para utilizar modelos preentrenados de Transformers.

Cargar modelos:

Se carga el pipeline de Stanza con los procesadores necesarios: tokenize, pos, lemma, depparse.
Se carga el tokenizer y el modelo de Transformers (en este ejemplo, se utiliza BERT para análisis de sentimiento).
Función extraer_caracteristicas:

Procesa el texto dado con el pipeline de Stanza para obtener información sintáctica.
Extrae las dependencias sintácticas de cada palabra en cada oración y las almacena en la variable dependencias.
Utiliza el modelo de Transformers para realizar el análisis de sentimiento del texto.
Calcula las probabilidades de cada etiqueta de sentimiento y obtiene la etiqueta predicha.
Almacena la etiqueta predicha en la variable sentimiento.
Devuelve un diccionario con las características extraídas.
Ejemplo de uso:

Se define un ejemplo de texto.
Se llama a la función extraer_caracteristicas para obtener las características del texto.
Se imprimen las características en la consola.
Se guardan las características en un archivo JSON.
Este código demuestra cómo combinar Stanza y Transformers para extraer características de un texto:

Stanza: proporciona información sintáctica detallada, como dependencias entre palabras.
Transformers: permite realizar tareas de procesamiento del lenguaje natural más avanzadas, como el análisis de sentimiento.
Puedes adaptar este código para extraer otras características o utilizar diferentes modelos de Transformers según tus necesidades específicas.

¡Claro! Vamos a desglosar la salida que obtuvimos en el código anterior:
...
Esta estructura JSON representa un diccionario en Python, y cada clave-valor representa una característica extraída del texto.

"dependencias": []

dependencias: Esta clave se refiere a las relaciones gramaticales entre las palabras en el texto.
[]: El valor es una lista vacía en este caso. Esto significa que para el texto proporcionado, no se encontraron dependencias significativas o el modelo de Stanza no pudo identificarlas. Las dependencias suelen ser pares de palabras donde una palabra depende sintácticamente de otra (por ejemplo, sujeto-verbo, adjetivo-sustantivo). Si hubiera encontrado dependencias, verías una lista de tuplas o diccionarios que describen estas relaciones.
"sentimiento": 0

sentimiento: Esta clave representa la polaridad del sentimiento expresado en el texto, según el modelo de Transformers utilizado (en este caso, BERT).
0: El valor 0 indica la etiqueta de sentimiento asignada al texto. El significado exacto de este valor depende de cómo se hayan entrenado las etiquetas de sentimiento en el modelo. Generalmente, un valor bajo (cercano a 0) podría indicar un sentimiento neutral o mixto, mientras que valores más altos o más bajos podrían indicar sentimientos positivos o negativos, respectivamente.
En resumen:

La salida indica que para el texto proporcionado:

No se encontraron dependencias sintácticas significativas: Esto podría deberse a la simplicidad de las oraciones, a la falta de ambigüedad o a limitaciones del modelo de Stanza.
El sentimiento del texto es neutral o mixto: El modelo de Transformers ha clasificado el texto como teniendo un sentimiento cercano a la neutralidad.
"""
...
"""¡Absolutamente! Aquí tienes un código que combina Stanza y Transformers para realizar un análisis más profundo del texto, incluyendo tanto el análisis morfosintáctico como el análisis de sentimientos:
...
Este código combina lo mejor de ambos mundos:

Stanza: Proporciona un análisis detallado de la estructura gramatical del texto, incluyendo partes del discurso, lemas y dependencias.
Transformers: Permite realizar tareas de alto nivel como el análisis de sentimientos, aprovechando modelos preentrenados como BERT.
Cambios clave:

Función analizar_texto_completo: Esta función engloba tanto el análisis morfosintáctico como el análisis de sentimientos.
Retorno: La función devuelve un diccionario con dos claves:
analisis_morfosintactico: Contiene la información morfosintáctica extraída por Stanza.
sentimiento: Contiene la etiqueta de sentimiento predicha por el modelo de Transformers.
Análisis de sentimientos: Se ha agregado el código para realizar el análisis de sentimientos utilizando BERT.
Beneficios:

Análisis completo: Obtienes una visión más completa del texto, incluyendo tanto la estructura gramatical como la polaridad del sentimiento.
Flexibilidad: Puedes personalizar el análisis agregando más características o utilizando diferentes modelos de Transformers.
Claridad: El código está bien estructurado y comentado para facilitar su comprensión.
Consideraciones:

Modelos de Transformers: Puedes experimentar con diferentes modelos de Transformers para obtener mejores resultados en diferentes tareas.
Idioma: Asegúrate de ajustar el idioma en el pipeline de Stanza y en el modelo de Transformers si estás trabajando con un idioma diferente al inglés.
Preprocesamiento: El preprocesamiento del texto puede influir significativamente en los resultados. Puedes agregar pasos de preprocesamiento adicionales, como la eliminación de stop words o la lematización.
"""
...
Importar bibliotecas: Se importan las bibliotecas necesarias:

stanza para el análisis lingüístico.
torch para trabajar con tensores.
AutoTokenizer y AutoModelForSequenceClassification de transformers para el análisis de sentimiento con BERT.
json para manejar archivos JSON.
Cargar modelos:

Se carga el pipeline de Stanza con los procesadores necesarios: tokenize, pos, lemma, depparse, ner.
Se carga el tokenizer y el modelo de BERT para el análisis de sentimiento.
Función analizar_texto:

Procesa el texto dado con el pipeline de Stanza.
Itera sobre cada oración en el documento.
Análisis Morfosintáctico: Extrae información sobre cada palabra: texto, parte del discurso (POS), lema y dependencias.
Extracción de Entidades Nombradas: Utiliza Stanza para identificar entidades nombradas (personas, lugares, organizaciones) en la oración.
Análisis de Sentimiento: Utiliza el modelo de BERT para clasificar el sentimiento de la oración y obtiene las probabilidades de cada clase.
Crea un diccionario para almacenar los resultados de la oración.
Agrega el diccionario de resultados a una lista.
Devuelve la lista de resultados.
Ejemplo de uso:

Se define un texto de ejemplo.
Se llama a la función analizar_texto para obtener los resultados.
Se imprimen los resultados en la consola.
Se guardan los resultados en un archivo JSON.
Salida:

El código imprimirá en la consola los resultados del análisis para cada oración, incluyendo:

Texto de la oración.
Análisis morfosintáctico (texto, POS, lema, dependencias).
Entidades nombradas.
Sentimiento (etiqueta y probabilidades).
Además, se creará un archivo JSON llamado analisis_completo.json que contendrá todos los resultados del análisis.
"""
...
Estructura clara: El código está organizado en funciones bien definidas y con comentarios explicativos.
Detección de voz pasiva: Se ha mejorado la detección de voz pasiva considerando verbos auxiliares y dependencias.
Reemplazo de entidades: Se reemplazan las entidades con su tipo entre corchetes para una mejor visualización.
Salida detallada: Se imprime información detallada sobre cada oración, incluyendo el análisis morfosintáctico, entidades, sentimiento, voz pasiva y el texto modificado.
Archivo JSON: Los resultados se guardan en un archivo JSON formateado para facilitar su análisis posterior."""